...
競合図解では、この前提と照らして **重なる点・ずれる点（勝ち筋の材料）** が読み取れるようにしています（ルールの詳細は `.claude/skills/diagram-personal/SKILL.md`）。
...
パーソナル図解スキルが有効な環境では、チャット欄で **「開始」** と入力すると、個人事業主向けAIコンサル／顧問に近い事業をしている**競合1件**のリサーチから図解 HTML 生成・surge.sh デプロイまで進みます（手順の詳細は `.claude/skills/diagram-personal/SKILL.md`）。

生成された図解は `output/` フォルダに保存されます。次回以降の「開始」では、このフォルダの内容を参照して**過去に扱った競合と被らない**よう選定します（ルールの詳細は `.claude/skills/diagram-personal/SKILL.md`）。
