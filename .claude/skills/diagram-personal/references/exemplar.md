...
| **公開 URL（ブラウザ確認用）** | https://eu-ai-act-personal-mar25-2026.surge.sh/ （他ファイルでは URL を書かない）。アンカー例: `#section-map` / `#section-three` / `#section-work` |
...
- **全体の地図**: `h2`（Lucide + 見出し）→ リード文 → `grid gap-4 md:grid-cols-2` のカード（番号付きタイトル + 短い説明）。**意味付けの正**は [SKILL.md](../SKILL.md) の「全体の地図」：**1テーマ用**に **3ブロック以内**、流れは **「定義 → 起き方・具体例 → 対処」** が一目でわかること。カードの**ラベル文言はテーマごとに毎回変える**（仕事／社会／講師の**固定3側面**に対応させる必要はない）。見本はレイアウトとカード数の型を示すだけ。
- **用語**: **1 セクション**に `term-explain` を縦に積む（見本は「先に押さえる用語」+ `lightbulb` の `h2`）。中身の語彙ルールは term-dictionary のみが正。
- **今日の3つの要点**: `check-circle` の **`h2` 文言は「今日の3つの要点」に統一**（旧称「まず押さえる3つ」等は使わない）→ リード → `space-y-4` の行（`badge-essential` / `badge-recommended` / `badge-optional` + 見出し + 補足）。各行に **一言要約** と **受講生・契約先への伝え方のヒント** を含める（見本の `#section-three`）。**3行の意味（①正体 ②個人と社会 ③守り方・伝え方）** の定義は [SKILL.md](../SKILL.md) の表のみを正とする（ここでは繰り返さない）。
- **本編各セクション**（**その1テーマについて**）: `section-card` ごとに `h2`（Lucide + タイトル）→ **学習者対話（`mb-6`）→ ナビゲーター対話（`mb-6`）** → 本文（箇条書き・`rounded-lg border` のまとめ枠・2 カラムのミニ図・`<details>` など、見本にあるパターンから選ぶ）→ 必要なら `text-xs` の参考リンク行。**セクション数の正**: **3〜5**（見本は **`section-work`〜`section-teach` の 4 本編**でレイアウト例を示す。必要なら増減）。`id` の命名は `section-*` でトピックに合わせてよい。
- **まとめ**: 見本どおり `section-summary` で、**[SKILL.md](../SKILL.md) の「まとめ（最終セクションの必須構成）」**（一言／個人と社会／講師・コンサル）を**本文または箇条書きで明示**したうえで、学習者の**自分の言葉の要約** → ナビゲーターの肯定・締め（台本の型は dialogue-personas）。
...
3. 「全体の地図」（見本: グリッドカード + 締めのたとえ1段落）
   └ **最大3ブロック**・**定義 → 起き方・具体例 → 対処**の流れがわかること（ラベルはテーマ依存。正は SKILL.md）

4. 用語（1 セクションに term-explain を縦積み）
   └ 「この図解では〇〇と呼ぶ」を必ず入れる（詳細は term-dictionary.md）

5. 今日の3つの要点（h2 文言はこの表記に統一）
   └ 必須 / 推奨 / 任意 などバッジで優先度（見本のカード行レイアウトに合わせる）。3行の意味は SKILL.md の表のみを正とする

6. 各トピック section-card（**その1テーマ**について **3〜5** セクションまで）
   └ h2（Lucide）→ 学習者対話 → ナビゲーター対話 → 本文・図・折りたたみ → 参考リンク行（必要に応じて）。見本は4本編セクションのレイアウト例（仕事→役割→なぜ今→講師）
...
- [ ] **1回につき AI リスクは1テーマだけ**（百科・全部入りにしていない）。冒頭に**選定理由**が分かる（[SKILL.md](../SKILL.md)「リサーチの固定」）
- [ ] **全体の地図**が **3ブロック以内**で、**定義 → 起き方・具体例 → 対処**の流れが読み取れる（ラベルはテーマに合わせている）
...
- [ ] **今日の3つの要点**があり、**見本と同じバッジ行レイアウト**になっている（各行に**受講生への伝え方のヒント**がある）。3行の意味は [SKILL.md](../SKILL.md) の表と一致
- [ ] 本編が**その1テーマについて** **3〜5** セクションの範囲に収まっている（見出しはテーマ依存でよい）
- [ ] **まとめ**で [SKILL.md](../SKILL.md) の「まとめ（最終セクションの必須構成）」**3点**（一言／個人と社会／講師・コンサル）がはっきりしている
...
- [ ] **リスクの骨子**と**対応策**が本文またはまとめで**一言以上**ずつ言語化されている。**受講生にどう言うか**のイメージが付いている
